...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.output(combined_heads), attention_scores, attention_weights
...
class TransformerEncoderBlock(nn.Module):
    ...
    def forward(self, x):
        attention_output, attention_scores, attention_weights = self.attention(x)
        x = self.attention_norm(x + attention_output)
        feed_forward_output = self.feed_forward(x)
        x = self.feed_forward_norm(x + feed_forward_output)
        return x, attention_scores, attention_weights


class TinyTransformerClassifier(nn.Module):
    def __init__(self, vocab_size, seq_len, d_model, num_heads, d_ff, num_classes):
        super().__init__()
    ...
    def forward(self, token_ids):
        ...
        encoder_output, _, _ = self.encoder_block(embeddings)
